...
public class CartServiceImpl implements CartService {
	...
	public List<Cart> getCartsByUser(Integer userId) {
		...
		for (Cart c : carts) {
			int availableStock = c.getProduct().getStock();

			// Sepet miktarı stoktan büyükse, maksimum stok miktarını kullan
			if (c.getQuantity() > availableStock) {
				c.setQuantity(availableStock);  // Stok miktarı kadar ekle
			}
